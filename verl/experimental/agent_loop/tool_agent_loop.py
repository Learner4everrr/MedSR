...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        agent_data._native_search_mode = "search" in getattr(agent_data, "_active_tools", {})

        if extra_info.get("force_search") and "search" in getattr(agent_data, "_active_tools", {}):
            await self._force_initial_search(agent_data, extra_info)
    ...
    async def _force_initial_search(self, agent_data: AgentData, extra_info: dict[str, Any]) -> None:
        """Prepend one Search-R1-like retrieval turn before the model answers."""
        query = str(extra_info.get("search_query") or extra_info.get("question") or "").strip()
        if not query:
            return
        ...
        tool_call = FunctionCall(
            ...
            tool_call_id=f"call_search_{uuid4().hex[:8]}",
        )
        agent_data.tool_calls = [tool_call]
        agent_data.messages.append(self._build_assistant_message("", agent_data))

        with simple_timer("forced_search", agent_data.metrics):
            tool_response, tool_reward, _ = await self._call_tool(tool_call, agent_data.tools_kwargs, agent_data)
        ...
        agent_data.metrics["forced_search_calls"] = 1.0

    async def _handle_pending_state(self, agent_data: AgentData, sampling_params: dict[str, Any]) -> AgentState:
        """Handle the pending state: prepare the prompt and start generation."""
        schemas = [] if getattr(agent_data, "_native_search_mode", False) else getattr(
            agent_data, "_active_tool_schemas", self.tool_schemas
        )
    ...
    async def _handle_generating_state(
        ...
        prev_prompt_ids = list(agent_data.prompt_ids)
        ...
        if "search" in getattr(agent_data, "_active_tools", {}):
            stop = sampling_params.get("stop") or []
            if isinstance(stop, str):
                stop = [stop]
            for stop_text in ("</search>", "<information>"):
                if stop_text not in stop:
                    stop = [*stop, stop_text]
            sampling_params = {**sampling_params, "stop": stop, "include_stop_str_in_output": True}
        ...
        native_search = await self._extract_native_search_from_response(agent_data.response_ids)
        active_tools = getattr(agent_data, "_active_tools", self.tools)
        search_tool_available = "search" in active_tools or "search" in self.tools
        if native_search is not None and search_tool_available:
            query, prefix_text = native_search
            trimmed_ids = await self.loop.run_in_executor(
                None, lambda: self.tokenizer.encode(prefix_text, add_special_tokens=False)
            )
            if trimmed_ids:
                agent_data.response_ids = trimmed_ids
                agent_data.prompt_ids = prev_prompt_ids + trimmed_ids
                agent_data.response_mask = agent_data.response_mask[:prev_response_mask_len] + [1] * len(trimmed_ids)
                if agent_data.response_logprobs:
                    kept = agent_data.response_logprobs[prev_response_logprobs_len : prev_response_logprobs_len + len(trimmed_ids)]
                    if len(kept) < len(trimmed_ids):
                        kept += [0.0] * (len(trimmed_ids) - len(kept))
                    agent_data.response_logprobs = agent_data.response_logprobs[:prev_response_logprobs_len] + kept
            agent_data.metrics["native_search_detected"] = agent_data.metrics.get("native_search_detected", 0.0) + 1.0
            agent_data.metrics["native_search_query_chars"] = float(len(query))
            agent_data.tool_calls = [
                FunctionCall(
                    name="search",
                    arguments=json.dumps({"query": query}, ensure_ascii=False),
                    tool_call_id="native_search",
                )
            ]
    ...
    async def _extract_native_search_from_response(self, response_ids: list[int]) -> tuple[str, str] | None:
        text = await self.loop.run_in_executor(None, self.tokenizer.decode, response_ids)
        match = re.search(r"<search>(.*?)(</search>|<information>|$)", text, flags=re.DOTALL | re.IGNORECASE)
        if not match:
            return None
        query = re.split(r"<information>|<answer>|</think>", match.group(1), flags=re.IGNORECASE)[0].strip()
        query = re.sub(r"\s+", " ", query)
        if not query:
            return None
        end_token = match.group(2) or ""
        if end_token.lower() == "</search>":
            prefix_text = text[: match.end(2)]
        else:
            prefix_text = f"{text[: match.start(1)]}{query}</search>"
        return query, prefix_text
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        if agent_data.tool_calls and all(call.tool_call_id == "native_search" for call in agent_data.tool_calls):
            info_parts = []
            ...
            agent_data.metrics["native_search_executed"] = agent_data.metrics.get("native_search_executed", 0.0) + float(
                len(responses)
            )
            agent_data.messages.append({"role": "user", "content": info_text})
            response_ids = await self.loop.run_in_executor(
                None, lambda: self.tokenizer.encode(info_text, add_special_tokens=False)
            )
    ...
    async def _call_tool(
        ...
        """
        active_tools = getattr(agent_data, "_active_tools", self.tools)
        if tool_call.name == "search" and tool_call.name not in active_tools and tool_call.name in self.tools:
