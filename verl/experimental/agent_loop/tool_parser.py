...
class HermesToolParser(ToolParser):
    ...
    def __init__(self, tokenizer) -> None:
        ...
        self.search_regex = regex.compile(r"<search>(.*?)(?:</search>|$)", regex.DOTALL | regex.IGNORECASE)
    ...
    async def extract_tool_calls(
        ...
        search_call = self._extract_search_tool_call(text, tools)
        if search_call is not None:
            content, function_call = search_call
            return content, [function_call]
        if self.tool_call_start_token not in text or self.tool_call_end_token not in text:
            return self._extract_bare_json_tool_call(text, tools)
    ...
    def _extract_search_tool_call(
        self, text: str, tools: list[OpenAIFunctionToolSchema] = None
    ) -> tuple[str, FunctionCall] | None:
        if not tools or "search" not in self._tool_names(tools):
            return None
        match = self.search_regex.search(text)
        if not match:
            return None
        query = regex.split(r"<information>|<answer>|</think>", match.group(1), flags=regex.IGNORECASE)[0].strip()
        query = regex.sub(r"\s+", " ", query)
        if not query:
            return None
    ...
    @staticmethod
    def _tool_names(tools: list[OpenAIFunctionToolSchema]) -> set[str]:
        names = set()
        for tool in tools:
    ...
    def _extract_bare_json_tool_call(
        self, text: str, tools: list[OpenAIFunctionToolSchema] = None
    ) -> tuple[str, list[FunctionCall]]:
        """Accept a bare OpenAI-style function-call JSON object.

        Qwen2.5-3B often emits the requested JSON object but drops the
        ``<tool_call>`` wrapper. Treat that as a tool call only when the name is
        one of the registered tools, so normal final answers are unaffected.
        """
        if not tools:
            return text, []
        tool_names = {tool.function.name for tool in tools}
        for candidate in self._json_objects(text):
            try:
    ...
    def _json_objects(text: str):
        """Yield balanced JSON-object substrings found anywhere in text."""
        start = text.find("{")
        while start >= 0:
            depth = 0
            in_string = False
