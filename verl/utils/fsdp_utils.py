...
def layered_summon_lora_params(fsdp_module) -> OrderedDict:
    ...
    def _clean_lora_key(key):
        key = key.replace("_fsdp_wrapped_module.", "").replace(".base_layer", "")
    ...
    for name, submodule in fsdp_module.named_modules():
        ...
        # Exclude nested FSDP children so their params are not gathered here and
        # again when their own unit is visited.
        ...
        try:
            with summon_ctx:
                ...
                try:
                    # With use_orig_params=False, named_parameters() exposes
                    # only FSDP flat params. state_dict() inside summon restores
                    # original module keys on compatible FSDP builds.
                    state_items = submodule.state_dict().items()
                except AttributeError as exc:
                    import logging

                    logging.getLogger(__name__).warning(
                        "layered_summon state_dict failed for %s (%s); falling back to full summon",
                        clean_prefix,
                        exc,
                    )
                    return OrderedDict()

                for n, p in state_items:
                    if any(n.startswith(f"{nn}.") for nn in nested_fsdp_names):
                        continue
                    clean_name = _clean_lora_key(n)
                    if "lora_" not in clean_name:
                        continue
                    if clean_prefix and not clean_name.startswith(f"{clean_prefix}."):
                        clean_name = f"{clean_prefix}.{clean_name}"
                    sub_state_dict[clean_name] = p
