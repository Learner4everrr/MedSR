...
export CORPUS_NAME="${CORPUS_NAME:-pubmed}"
export DATASETS_OVERRIDE="${DATASETS_OVERRIDE:-medqa_usmle headqa pubmedqa_labeled}"
export OUT_ROOT_BASE="${OUT_ROOT_BASE:-$VERL_ROOT/outputs/formal_pubmed_native_lora_3models_500step_promptfix}"
...
export PROJECT_NAME="${PROJECT_NAME:-verl_biomed_search_lora_formal_pubmed}"
...
run_model() {
  ...
  echo "===== START $model_tag on pubmed: $model_path ====="
  "$VERL_ROOT/scripts/biomed/run_textbooks_lora_flash.sh"
  echo "===== DONE $model_tag on pubmed ====="
}
