...
export CORPUS_NAME="${CORPUS_NAME:-statpearls}"
export DATASETS_OVERRIDE="${DATASETS_OVERRIDE:-medqa_usmle headqa pubmedqa_labeled}"
export OUT_ROOT_BASE="${OUT_ROOT_BASE:-$VERL_ROOT/outputs/formal_statpearls_native_lora_3models_500step_promptfix}"
...
export PROJECT_NAME="${PROJECT_NAME:-verl_biomed_search_lora_formal_statpearls}"
...
run_model() {
  ...
  echo "===== START $model_tag on statpearls: $model_path ====="
  "$VERL_ROOT/scripts/biomed/run_textbooks_lora_flash.sh"
  echo "===== DONE $model_tag on statpearls ====="
}
