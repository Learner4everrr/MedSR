...
LOG_ROOT="${LOG_ROOT:-$VERL_ROOT/outputs/baselines_prompt_only_3methods_logs}"
...
worker() {
  ...
  while IFS=$'\t' read -r baseline model_tag model_path dataset; do
    if (( line_no % ${#GPUS[@]} == worker_id )); then
      run_task "$gpu" "$baseline" "$model_tag" "$model_path" "$dataset"
    fi
    line_no=$((line_no + 1))
  done < "$TASK_FILE"
}
...
root = Path("outputs/baselines_prompt_only_3methods")
rows = []
